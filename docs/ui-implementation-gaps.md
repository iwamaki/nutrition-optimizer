...
- [x] 「献立を生成」ボタン
- [x] 期間トグル（今日/週間）
- [x] **期間トグルに「月間」を追加** - 設計では3択（今日/週間/月間）✅
...
- [x] 共有機能
- [x] 進捗表示
- [x] **期間表示を日付形式に変更** - 「1/15(月)〜1/17(水) 2人分」✅
- [x] **「手持ち食材を除いた不足分のみ」注記の追加** ✅
- [x] **サマリーの詳細化** - 「全X品目 / 購入済み: X品 / 残り: X品」✅
...
- [x] カロリー目標設定
- [x] アレルゲン除外設定
- [x] **タンパク質目標の追加** - 設計では「60-100g」✅
- [x] **その他栄養素目標の追加** - 設計では「その他栄養素...」→詳細画面 ✅
...
6. **[P2-3] ホーム画面: 月間トグル追加** ✅ 完了
   - ファイル: `home_screen.dart`
   - 実装内容:
     - 期間トグルに「月間」を追加
     - 期間ラベルの動的表示

7. **[P2-4] 買い物リスト: 表示改善** ✅ 完了
   - ファイル: `shopping_screen.dart`, `shopping_provider.dart`
   - 実装内容:
     - 日付範囲表示（「1/15(月)〜1/17(水) 2人分」形式）
     - 「手持ち食材を除いた不足分のみ表示」注記
     - サマリーの詳細化（全X品目/購入済み/残り）
...
9. **[P3-2] 設定画面: 栄養目標の拡充** ✅ 完了
   - ファイル: `settings_screen.dart`, `settings_provider.dart`
   - 実装内容:
     - タンパク質目標の設定UI（スライダー）
     - その他栄養素の詳細表示ダイアログ
